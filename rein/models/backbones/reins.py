...
text_cls = class_name_dict.values()
...
class Reins(nn.Module):
    def __init__(
        ...
        self.text_embeddings = clip.tokenize(text_cls).to(self.device)
        with torch.no_grad():
            self.text_features = self.clip_model.encode_text(self.text_embeddings)
    ...
    def return_auto(self, feats):
        if self.link_token_to_query:
            ...
            return feats, querys, self.learnable_tokens, self.text_features
        else:
            return feats
    ...
    def forward(
        self, feats: Tensor, layer: int, B: int, H: int, W: int, imgs=None, labels=None, batch_first=False, has_cls_token=True
    ) -> Tensor:
        if batch_first:
        ...
        if B > 1:
            cls_ids = list(set(labels.view(-1).cpu().int().tolist()))
            if 255 in cls_ids:
                cls_ids.remove(255)
            with torch.no_grad():
                sub_text_features = self.text_features[cls_ids]
            tt_attn = torch.einsum("nc,mc->nm", tokens, sub_text_features.to(torch.float32))
            ttt_attn = torch.einsum("nm,mc->nc", tt_attn, sub_text_features.to(torch.float32))
            tokens = tokens+ttt_attn
...
class LoRAReins(Reins):
    ...
    def return_auto(self, feats):
        if self.link_token_to_query:
            ...
            querys = self.merge(tokens.flatten(-2, -1))
            return feats, querys, self.learnable_tokens_a @ self.learnable_tokens_b, self.text_features
        else:
            return feats
